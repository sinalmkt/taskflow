...
function renderClients(){
  ...
  for(const c of clients){ const n = tmpl.content.cloneNode(true); n.querySelector('.title').textContent=c.name; n.querySelector('.select').addEventListener('click', () => { activeClientId=c.id; renderDrive(); showToast('Cliente selecionado: '+c.name,'success',2500); }); n.querySelector('.del').addEventListener('click', () => { clients=clients.filter(x=>x.id!==c.id); save(STORAGE.clients, clients); if(activeClientId===c.id) activeClientId=clients[0]?.id||null; renderClients(); renderDrive(); showToast('Cliente excluído','success'); }); list.appendChild(n); }
  if (!clients.length){ const e=document.createElement('div'); e.className='item'; e.textContent='Sem clientes.'; list.appendChild(e); }
  document.getElementById('addClient').onclick = () => { const name = document.getElementById('clientName').value.trim(); if(!name){ showToast('Digite um nome para o cliente','error'); return; } const c = { id: crypto.randomUUID?.() || String(Math.random()).slice(2), name, contact: document.getElementById('clientContact').value.trim(), notes: document.getElementById('clientNotes').value.trim(), files: [], createdAt: new Date().toISOString() }; clients.push(c); save(STORAGE.clients, clients); document.getElementById('clientName').value=''; document.getElementById('clientContact').value=''; document.getElementById('clientNotes').value=''; renderClients(); showToast('Cliente adicionado','success'); };
}
...
function renderDrive(){ const c = clients.find(x => x.id===activeClientId) || null; activeClientName.textContent = c ? c.name : 'Selecione um cliente'; activeClientContact.textContent = c?.contact || ''; uploadBtn.disabled = !c; removeClientBtn.disabled = !c; dropZone.classList.toggle('disabled', !c); fileGrid.innerHTML=''; if(!c) return; removeClientBtn.onclick = () => { clients = clients.filter(x => x.id!==c.id); save(STORAGE.clients, clients); activeClientId = clients[0]?.id || null; renderClients(); renderDrive(); showToast('Cliente excluído','success'); }; const tmpl = document.getElementById('fileCardTemplate'); for(const f of c.files){ const node = tmpl.content.cloneNode(true); const th = node.querySelector('.thumb'); if (f.type && f.type.startsWith('image/')){ const img = document.createElement('img'); img.src = f.dataUrl; th.appendChild(img); } else { th.innerHTML = '<div class="muted small">Arquivo<br>'+(f.type||'')+'</div>'; } node.querySelector('.file-name').textContent = f.name; const sizeKB = f.size/1024; node.querySelector('.file-size').textContent = sizeKB >= 1024 ? (sizeKB/1024).toFixed(2)+' MB' : sizeKB.toFixed(0)+' KB'; const [aOpen, aDown] = node.querySelectorAll('a'); aOpen.href = f.dataUrl; aDown.href = f.dataUrl; aDown.download = f.name; node.querySelector('.remove').addEventListener('click', () => { c.files = c.files.filter(x => x.id!==f.id); save(STORAGE.clients, clients); renderDrive(); showToast('Arquivo removido','success'); }); fileGrid.appendChild(node); } if (c.files.length===0){ const e=document.createElement('div'); e.className='item'; e.textContent='Nenhum arquivo enviado.'; fileGrid.appendChild(e); } }
